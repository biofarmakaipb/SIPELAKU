...
class TransaksiProyekController extends Controller
{
    ...
    public function editPengajuanDana(Request $request){
        try{
            ...
            $transaksiProyek = TransaksiProyek::where('id_transaksi',$idTransaksi)->first();
            $transaksi->nominal = $subTotal;
            $transaksi->save();
            $transaksiProyek->keterangan = $keterangan;
            $transaksiProyek->perkiraan_biaya = $perkiraanBiaya;
            $transaksiProyek->jumlah = $jumlah;
            $transaksiProyek->unit = $unit;
            $transaksiProyek->save();
            return ['status'=>true];
        }
        catch(Exception $err){
    ...
    public function tambahItemPengajuanDana(Request $request){
        try{
        ...
        try{
            ...
            if(!$transaksi->isEmpty()){
                foreach($transaksi as $transRow){
                    $data->push([
                        'id'=> $transRow->id,
                        'id_pegawai'=> $transRow->pegawai->id,
                        'id_proyek'=> $transRow->transaksiProyek[0]->proyek->id,
                ...
                foreach($dataGrouped as $dataTanggal){
                    foreach($dataTanggal as $dataKategori){
                        ...
                        $returnedData->push([
                            ...
                            'id_pegawai'=> $dataKategori[0]['id_pegawai'],
                            'pegawai'=> $dataKategori[0]['pegawai'],
                            'kategori'=> $dataKategori[0]['kategori'],
                            'id_proyek'=> $dataKategori[0]['id_proyek'],
                            'total'=>$sum,
                            'item'=> $dataKategori
                        ]);
                    }
